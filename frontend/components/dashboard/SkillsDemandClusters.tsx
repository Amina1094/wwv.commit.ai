...
type ClusterKey = "technology" | "healthcare" | "defense";
...
export function SkillsDemandClusters({
  ...
  return (
    <div className="grid gap-3 lg:grid-cols-3">
      {(Object.keys(clusters) as ClusterKey[]).map((k) => {
          ...
          >
            <CardHeader className="pb-2">
              <CardTitle className="flex items-center justify-between text-sm font-semibold text-slate-200">
                ...
                <span className={`rounded-md border px-2 py-0.5 text-[10px] uppercase tracking-[0.16em] ${CLUSTER_STYLES[k].chip}`}>
            ...
            <CardContent className="space-y-2">
              {items.length === 0 ? (
                <p className="text-xs text-slate-500">No clustered skills found.</p>
              ) : (
                items.map((it) => {
